...
const FormRight = (props) => {
  const [state, setState] = useState({ tempValue: "", isActive: false, sort: ""});
    ...
    if (state.isActive === true && state.sort === "") {
      result = (
        <div className="sort-list" id="sort">
          <button
            type="button"
            onClick={()=>setState({...state,sort: "nameAZ", isActive: false})}
          >
            <i className="fas fa-sort-alpha-down" /> Tên A-Z{" "}
          </button>
          <button
            type="button"
            onClick={()=>setState({...state,sort: "nameZA", isActive: false})}
          >
          ...
          <button
            type="button"
            onClick={() =>setState({...state,sort: "statusActive", isActive: false})}
          >
            Trang thai kich hoat{" "}
          </button>
          <button
            type="button"
            onClick={() =>setState({...state,sort: "statusHide", isActive: false})}
          >
            Trang thai an{" "}
          </button>
        </div>
      ...
      return (
        <div className="sort-list" id="sort">
          ...
          <button
            type="button"
            onClick={() =>
              setState({ sort: "statusHide", isActive: false })
            }
          >
            Trang thai an{" "}
          </button>
  ...
  const hide = () => {
    const result = [];
    const { dataUserProps } = props;
    dataUserProps.forEach((element) => {
      if (element.value === 2) {
        result.push(element);
      }
    });
  ...
  const mappingDataUser = () => {
    ...
    if (props.dataUserProps !== null && state.sort === "") {
      return props.dataUserProps.map((value, key) => (
        ...
        key={key} 
          ...
          editFunClick={(user) => props.editFun(value)}
          userName={value.name}
    ...
    if (dataUserProps !== null && state.sort === "hide") {
      const result = [];
      dataUserProps.forEach((element) => {
    ...
    if (dataUserProps !== null && sort === "active") {
      const result = [];
      dataUserProps.forEach((element) => {
